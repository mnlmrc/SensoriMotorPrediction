...
%
%	DATA_OUT = FILTER_DOUBLE_PASS(DATA_IN, METHOD, 'fc', FC, 'fs', FS, 'order', ORDER)
%	filters all floating-point channel data in the structure DATA_IN. The
%	data are all double-pass filtered using an nth order filter
%	(double-pass of nth order filter implemented for zero-lag filtering),
%	with a 3db cutoff of the 2*nth order final filter at FC Hz.  
%
%	The METHOD parameter can either be 'standard' or 'enhanced'.  The
%	'standard' method uses reflection of data about the end-points to
%	minimize starting and end transients  (similar to the filtfilt.m
%	function in the Signal Processing toolbox). The 'enhanced' method
%	builds upon the standard method to provide a better reflection,
%	particularly when a data stream starts or ends at the peak of a noise
%	spike. 
% 
%	If FS is not supplied, then the sampling frequency is extracted from
%	DATA_IN.  
%
%	If ORDER is not supplied, then a 3rd order filter is used for each
%	pass. Valid values of order are 1, 2 or 3 (i.e. for overall double-pass
%	filter of 2, 4 or 6 order). 
%
...
%	defined by B and A.  
% 
%	The input structure DATA_IN	can in one of two forms, based on exam_load:
%   data = exam_load;
%   dataNew = filter_double_pass(data)
%     OR
%   dataNew.c3d = filter_double_pass(data(ii).c3d)
%
%   Copyright 2009-2022 BKIN Technologies Ltd
...
while x <= length(varargin)
	...
	elseif strncmpi(varargin{x}, 'order', 5)
		x = x + 1;
		filter.order = varargin{x};
	end
    x = x + 1;
end
...
if isfield(dataIn, 'c3d')
	...
	% exam_load), then filter each exam file, one at a time.
...
if ~filter.coeffSpecified
	...
	[filter.B, filter.A] = CreateFilterCoeffForDblpass(filter.fc, filter.fs, filter.order);
...
function [B A] = CreateFilterCoeffForDblpass(fcFinal, fs, order)
% This function creates the coefficients for an nth order Butterworth
% filter which will be used subsequently as the basis of a double-pass
% (zero-lag) 2*nth order filter.  The input fc_final is the desired 3db
% cutoff frequency (Hz) of the double-pass order filter.  fs is the
% sampling frequency (Hz). 
%
% Only orders of 1-3 are presently supported.
%
% For a final 2*nth order filtering with a 3db cuttoff of fc, we need the
% nth order butterworth filter to have 1.5db cutoff at fc (because passing
% a given filter 2x over data_in will double the attenuation at all
% frequencies).  For 3rd order filters, the 3db cutoff frequency is
% ~1/0.864 above the 1.5 db attenuation (the correction factor is different
% for different order filters, e.g. it is ~0.802 for 2nd order filters).
% Note: this correction factor is dependent on fc/fs. The following
% correction factors were estimate for fc = 40 Hz and fs = 1000 Hz.

switch order
	case 1
		correction = 0.644;		% For a 1st order filter
	case 2
		correction = 0.803;		% For a 2nd order filter
	otherwise
		correction = 0.864;		% For a 3rd order filter
end
...
switch order
	case 1
		% for 1st order... (equations verified by comparing results to Filter Free...)
		A0 =   2 + wcw*T;
		A1 =  -2 + wcw*T;
		...
		A = [A0 A1] / A0;
	case 2
		A0 =  4 + 2 * sqrt(2)*wcw*T + 1*wcw^2*T^2;
		A1 = -8 - 0 * sqrt(2)*wcw*T + 2*wcw^2*T^2;
		A2 =  4 - 2 * sqrt(2)*wcw*T + 1*wcw^2*T^2;

		B = [1 2 1]*wcw^2*T^2 / A0;
		A = [A0 A1 A2] / A0;
	otherwise

...
if isfield(dataIn, 'c3d')
    ...
    for jj = 1:length(dataIn)
        ...
        was_method_run(dataOut(jj).c3d, 'KINARM_add_subject_inertia', true);
        was_method_run(dataOut(jj).c3d, 'KINARM_add_friction', true);
        was_method_run(dataOut(jj).c3d, 'KINARM_add_trough_inertia', true);
        was_method_run(dataOut(jj).c3d, 'filter_double_pass', true);
    end
else
	...
	for ii = 1:length(dataIn)
		for jj = 1:2
			...
			if isfield(dataIn(ii), [side2 '_HandX']) && ((isfield(dataIn(ii).([side '_KINARM']), 'IS_PRESENT') && dataOut(ii).([side '_KINARM']).IS_PRESENT) || (isfield(dataIn(ii).([side '_KINARM']), 'L1_I')))
                ...
                if isfield(dataIn(ii), [side2 '_RecTorque_M1'])
                    torque_m1 = dataIn(ii).([side2 '_RecTorque_M1']);
                    torque_m2 = dataIn(ii).([side2 '_RecTorque_M2']);
                else
                    torque_m1 = dataIn(ii).([side2 '_M1TorCMD']);
                    torque_m2 = dataIn(ii).([side2 '_M2TorCMD']);                    
                end
                
				if isfield(dataIn(ii), [side2 '_M1TorFRC']) && isfield(dataIn(ii), [side2 '_M2TorFRC'])
					M1TorApp = torque_m1 + dataIn(ii).([side2 '_M1TorFRC']);
					M2TorApp = torque_m2 + dataIn(ii).([side2 '_M2TorFRC']);
				else
					if strcmp(examFilename, 'legacy')

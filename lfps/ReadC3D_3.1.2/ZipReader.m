...
function [openFS, fsRoot] = openZipFileSystem(zip_name)
    ...
    if ~ff.exists()
        error(['The requested file does not exist: ' char(ff.getAbsolutePath())]);
    end
    ...
    try
        uri = java.net.URI.create(['jar:' char(ff.toURI().toString())]);
        disp('\n')
        disp(uri)
        openFS = java.nio.file.FileSystems.newFileSystem(uri, java.util.HashMap());
    catch ME
        disp(ME)

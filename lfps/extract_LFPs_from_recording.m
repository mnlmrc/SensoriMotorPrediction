...
pinfo = dload(fullfile(baseDir, 'Marco/SensoriMotorPrediction/participants.tsv'));
...
for m = 1:length(monkeys)
    ...
    for num_recording = recordings'
        ...
        outDir = fullfile(baseDir, 'Marco/SensoriMotorPrediction/LFPs/', monkey);
        if ~isfolder(outDir)
            mkdir(outDir);
        end
        ...
        for roi = 1:length(regions)
            ...
            % if isempty(ff) 
            %     ff = dir(fullfile(fileparts(lfp_parent), '*lf.bin')); 
            % end
            % 
            % if isempty(ff)

...
outDir = fullfile(baseDir, 'Marco/SensoriMotorPrediction/LFPs/', monkey);              % change if you want a different output dir
...
brainID = []; brainCoords = [];
ks_dir = [];
cand = dir([loadDir sprintf('/%s_g0/%s_g0_imec0/kilosort4', session, session)]);
if ~isempty(cand), ks_dir = cand(1).folder; end
if isempty(ks_dir)
    cand = dir([loadDir sprintf('%s_g0/%s_g0_imec0/sorted', session, session)]);
    if ~isempty(cand), ks_dir = cand(1).folder; end
end
...
neuralSync = cell(1,2);
neuralSync{2} = []; % no myo
sync_file = dir([loadDir sprintf('/%s_g0/%s_g0_imec0/sync.mat', session, session)]);

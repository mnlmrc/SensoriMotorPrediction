...
def make_searchlight(sn, experiment='smp2'):
    path_surf = os.path.join(gl.baseDir, experiment, gl.surfDir, f'subj{sn}')
    ...
    mask = [os.path.join(gl.baseDir, experiment, gl.roiDir, f'subj{sn}', f'Hem.{H}.nii') for H in gl.Hem]
    savedir = os.path.join(gl.baseDir, experiment, gl.roiDir, f'subj{sn}')

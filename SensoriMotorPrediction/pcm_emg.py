...
def calc_G(experiment='smp0', sns=None):
    path_emg = os.path.join(gl.baseDir, experiment, 'emg', )
    path_behav = os.path.join(gl.baseDir, experiment, gl.behavDir)
    ...
    G = np.zeros((len(sns), len(epochs), 8, 8))
    for s, sn in enumerate(sns):
        ...
        dat = pd.read_csv(os.path.join(path_behav, f'subj{sn}', f'{experiment}_{sn}.dat'), sep='\t')
...
def correlation(experiment='smp0', sns=None):

...
import time
import PcmPy as pcm
from SensoriMotorPrediction.util import prewhiten_with_lstsq_residuals
    ...
    for bl in blocks:
        ...
        for ons, onset in enumerate(stimOnset):
            for i, w in enumerate(win):
                ...
                force_tmp = mov[start:end, ch_idx]
                force_der = np.abs(np.gradient(force_tmp, 1 / gl.fsample_mov, axis=0)).mean(axis=0)
                force_avg = force_tmp.mean(axis=0)

                for j, f in enumerate(fingers):
                    force_dict[f'{f}{i}'].append(force_avg[j])
                    force_dict[f'{f}_der{i}'].append(force_der[j])
...
def calc_G_force(dat, experiment='smp2', prewhiten=True):
    ...
    G_force = np.zeros((dat.sn.nunique(), 5, 5))
    G_force_der = np.zeros_like(G_force)
    for s, sn in enumerate(dat.sn.unique()):
        dat_s = dat[dat.sn == sn]
        #dat_s = dat_s.groupby(['BN', 'cue']).mean(numeric_only=True).reset_index()
        cond_vec = dat_s['cue'].map(gl.regressor_mapping) #+ ',' + force['stimFinger']
        part_vec = dat_s['BN']

        force = dat_s[['thumb0', 'index0', 'middle0', 'ring0', 'pinkie0']].to_numpy()
        force_der = dat_s[['thumb_der0', 'index_der0', 'middle_der0', 'ring_der0', 'pinkie_der0']].to_numpy()

        if prewhiten:
            force = prewhiten_with_lstsq_residuals(force, part_vec)
            force_der = prewhiten_with_lstsq_residuals(force_der, part_vec)

        G_force[s], _ = pcm.est_G_crossval(force, cond_vec, part_vec, X=pcm.matrix.indicator(part_vec))
        G_force_der[s], _ = pcm.est_G_crossval(force_der, cond_vec, part_vec, X=pcm.matrix.indicator(part_vec))

    return G_force, G_force_der

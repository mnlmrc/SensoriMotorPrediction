...
def plot_binned_cue(fig, ax, dat, x='cue', y=None, markersize=2, jitter=.2, show_individuals=True):
    ...
    ax.set_xlabel('')
    handles = [Patch(facecolor=c, edgecolor='none', label=l) for l, c in zip(order, color)]
    ax.legend(handles=handles, frameon=False, bbox_to_anchor=(1, .5), loc='center left', fontsize=8)
...
def plot_binned_finger_cue(fig, axs, dat, x='cue', y=('index0', 'ring0'), finger=('nogo', 'nogo'), 
    ...
    palette0 = list(gl.colour_mapping.values())[5:9]
    palette1 = list(gl.colour_mapping.values())[9:13]

    sb.pointplot(dat[dat['stimFinger'] == finger[0]], x=x, y=y[0], ax=axs[0], errorbar='se', palette=palette0, order=order0, lw=2)
    sb.pointplot(dat[dat['stimFinger'] == finger[1]], x=x, y=y[1], ax=axs[1], errorbar='se', palette=palette1, order=order1, lw=2)

    if show_boxplot:
        sb.boxplot(dat[dat['stimFinger'] == finger[0]], x=x, y=y[0],  ax=axs[0], fill=False, palette=palette0, order=order0, showfliers=False, linewidth=1)
        sb.boxplot(dat[dat['stimFinger'] == finger[1]], x=x, y=y[1], ax=axs[1], fill=False, palette=palette1, order=order1, showfliers=False, linewidth=1)
    ...
    # combine labels and colors
    labels = order0 + order1
    colors = palette0 + palette1

    handles = [Patch(facecolor=c, edgecolor='none', label=l) for l, c in zip(labels, colors)]
...
def plot_heatmap_transitions(fig, ax, probs, chi2, p, name):

    def diverging_cmap(n=256):
        """Green (below expected) → white → coral (above expected)."""
        green  = np.array([15/255, 110/255, 86/255, 1.0])
        white  = np.array([1.0, 1.0, 1.0, 1.0])
        coral  = np.array([211/255, 85/255, 48/255, 1.0])
        half   = n // 2
        lo = np.linspace(green, white, half)
        hi = np.linspace(white, coral, n - half)
        colors = np.vstack([lo, hi])
        return mcolors.LinearSegmentedColormap.from_list("div", colors)

    """Draw a single annotated heatmap on ax."""
    n_rows, n_cols = probs.shape
    expected = 1.0 / n_cols          # uniform baseline

    # Centre the colormap on the expected value
    vmin = min(probs.values.min(), expected - 0.05)
    vmax = max(probs.values.max(), expected + 0.05)
    # Symmetric around expected
    half_range = max(abs(vmax - expected), abs(expected - vmin))
    vmin_sym = expected - half_range
    vmax_sym = expected + half_range

    cmap = diverging_cmap()
    im = ax.imshow(probs.values, cmap=cmap, vmin=vmin_sym, vmax=vmax_sym, aspect="auto")

    # Axis labels
    ax.set_xticks(range(n_cols))
    ax.set_yticks(range(n_rows))
    ax.set_xticklabels(probs.columns, rotation=30, ha="right", fontsize=9)
    ax.set_yticklabels(probs.index, fontsize=9)
    ax.set_xlabel("Next trial", fontsize=9)
    ax.set_ylabel("Current trial", fontsize=9)
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            v = probs.values[i, j]
            diff = v - expected
            color = "white" if abs(diff) > half_range * 0.5 else "black"
            ax.text(j, i, f"{v:.2f}", ha="center", va="center", fontsize=9, color=color)

    # Title with significance marker
    sig_str = f"p={p:.3f} *" if p < 0.05 else f"p={p:.3f}"
    ax.set_title(f"{name}  (χ²={chi2:.1f}, {sig_str})", fontsize=10, pad=8)

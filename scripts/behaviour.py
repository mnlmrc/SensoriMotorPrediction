...
if __name__ == '__main__':
    ...
    dat.loc[(dat["GoNogo"] == "go") & (dat["cue_prev2"].isna() | (prev_gonogo == "nogo")), "cue_prev2"] = np.nan

    # cue_prev1 column for carry over effect
    dat["stimFinger_prev1"] = dat.groupby("BN")["stimFinger"].shift(1)
    prev_gonogo = dat.groupby("BN")["GoNogo"].shift(1)
    dat.loc[(dat["GoNogo"] == "go") & (dat["stimFinger_prev1"].isna() | (prev_gonogo == "nogo")), "stimFinger_prev1"] = np.nan

    # cue_prev2 column for carry over effect
    dat["stimFinger_prev2"] = dat.groupby("BN")["stimFinger"].shift(2)
    prev_gonogo = dat.groupby("BN")["GoNogo"].shift(2)
    dat.loc[(dat["GoNogo"] == "go") & (dat["stimFinger_prev2"].isna() | (prev_gonogo == "nogo")), "stimFinger_prev2"] = np.nan
